...
Only read existing code as a template when no skill covers the case. The canonical caller reference is `lib/src/app/src/registered_app.dart` (`RegisteredApp.fetchAll`/`fetch` for queries, `registerWithoutCustomization` for mutations with decoded results, `grantAccessToApp` for bool mutations) and `AvailableApp.fetchAll`.
...
8. **Single-item fetch callers use list-first parsing.** Backend listing queries ALWAYS return `result` as an array, even when filtered by ID for single-item `fetch()` methods. The `fromJsonResult` decoder receives the raw `result` payload: cast as `List<dynamic>?` first, check null/empty (log warning, return null), then decode `.first`. Never cast the payload directly to Map in single-item fetchers — this causes runtime cast exceptions when the backend returns an array.
11. **Callers use `connector.query`/`connector.mutate` — never `connector.perform` directly.** `perform()` is the low-level primitive. Queries (`GqlQuery`) go through `connector.query<T>(gql, decoder)`, mutations (`GqlMutation`) through `connector.mutate<T>(gql, decoder)`. Both return `ApiResponse<T, Map<String, dynamic>>` with status/errors/result already decoded — no manual `response.data` digging, no manual `ApiStatus.fromJson`. Fetchers: `if (response.status != .ok) { onResponse?.call(response.status.toJson()); return <safe default>; } return response.result ?? <default>;`. Result-bearing mutations (register/edit/save-style) return a record `(ApiStatus, Map<String, dynamic>?, Model?)` with three-tier handling: `== .internalError` → `(ApiStatus.internalError, null, null)`; `!= .ok` → `(response.status, response.errors, null)`; ok → `(response.status, response.errors, response.result)`; catch → `(ApiStatus.internalError, null, null)`. This is how Layrz components consume callers. Decoders are file-level private helpers (e.g. `_registeredAppDecoder(Object? json)`) — single-object for mutation results, list-shaped for listing queries.
12. **Every connector method is wrapped in coverage markers.** `// coverage:ignore-start` immediately before the method's dartdoc and `// coverage:ignore-end` immediately after its closing brace — network code is excluded from coverage (`make coverage` and CI strip these regions via `tool/strip_ignored_coverage.dart`). Keep pairs balanced.
13. **Dot shorthands are house style in callers.** Use `type: .id` / `.string` / `.uuid` on GqlVariable and `response.status != .ok` — not the fully qualified enum names.
