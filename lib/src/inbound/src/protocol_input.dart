...
// InboundProtocolInput moved to layrz_sdk and is re-exported from
// package:layrz_models/layrz_models.dart. The definition below is commented
// out (not deleted) for reviewability; it stays retrievable until the
// migration is confirmed and the developer reviews the surface change this
// re-export introduces (notably: the SDK's `dynamicIcon` is nullable
// `AvatarInput?` here it was `required AvatarInput`; nested fields such as
// `requiredFields`, `flespiAcl`, `webhookStructure`, `commandsStructure`,
// `configStructure` use the SDK's read-model types instead of this
// package's local `*Input` nested types; and the `color` default changed
// from `Colors.blue` to `Color(0xFF2196F3)`).
//
// @unfreezed
